(function () {
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        currentProductId = window.MEDXONE.qs("id");
        loadProduct();
        if (window.lucide) window.lucide.createIcons();

        // Real-time polling — auto-update product detail if data changes
        if (currentProductId) {
            window.MEDXONE.poll("/products/" + encodeURIComponent(currentProductId), POLL_INTERVAL, function (p) {
                if (p && p.id) {
                    document.title = p.name + " — MEDXONE SYNERGY";
                    document.getElementById("crumbName").textContent = p.name;
                    document.getElementById("productContainer").innerHTML = renderProduct(p);
                    wireQuoteForm(p);
                    if (window.lucide) window.lucide.createIcons();
                }
            });
    ...
    function renderProduct(p) {
        ...
        var imageContent = img
            ...
            : '<div class="no-image-placeholder no-image-placeholder-lg" data-testid="no-image-placeholder"><i data-lucide="image-off" width="48" height="48"></i><span>No image available</span></div>';
